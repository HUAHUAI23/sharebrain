...
> = {
  [KEYS.audio]: {
    ...
    title: () => m.editor_media_insert_audio(),
    tooltip: () => m.editor_media_audio(),
  },
  [KEYS.file]: {
    accept: ['*'],
    icon: <FileUpIcon className="size-4" />,
    title: () => m.editor_media_insert_file(),
    tooltip: () => m.editor_media_file(),
  },
  [KEYS.img]: {
    accept: ['image/*'],
    icon: <ImageIcon className="size-4" />,
    title: () => m.editor_media_insert_image(),
    tooltip: () => m.editor_media_image(),
  },
  [KEYS.video]: {
    accept: ['video/*'],
    icon: <FilmIcon className="size-4" />,
    title: () => m.editor_media_insert_video(),
    tooltip: () => m.editor_media_video(),
  },
};
          ...
          >
            <DropdownMenuGroup>
              <DropdownMenuItem onSelect={() => openFilePicker()}>
                {currentConfig.icon}
                {m.editor_media_upload_from_computer()}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => setDialogOpen(true)}>
                <LinkIcon />
                {m.editor_media_insert_via_url()}
              </DropdownMenuItem>
            </DropdownMenuGroup>
          </DropdownMenuContent>
        </DropdownMenu>
...
function MediaUrlDialogContent({
  ...
  const embedMedia = React.useCallback(() => {
    if (!isUrl(url)) return toast.error(m.editor_media_invalid_url());
  ...
  return (
    <>
      ...
      <AlertDialogFooter>
        <AlertDialogCancel>{m.editor_media_cancel()}</AlertDialogCancel>
        <AlertDialogAction
          onClick={(e) => {
            e.preventDefault();
            embedMedia();
