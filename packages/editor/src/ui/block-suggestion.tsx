          ...
          <div className="flex flex-col gap-2">
            {suggestion.type === 'remove' &&
              getRemoveSummaryItems(suggestion.text!).map((text, index) => (
                <div key={index} className="flex items-center gap-2">
                  <span className="text-muted-foreground text-sm">{m.editor_suggestion_delete()}</span>
            ...
            {suggestion.type === 'insert' &&
              suggestionText2Array(suggestion.newText!).map((text, index) => (
                <div key={index} className="flex items-center gap-2">
                  <span className="text-muted-foreground text-sm">{m.editor_suggestion_add()}</span>

                  <span key={index} className="text-sm">
                    {text || m.editor_suggestion_line_breaks()}
                  </span>
                </div>
              ))}
                ...
                {suggestionText2Array(suggestion.newText!).map(
                  (text, index) => (
                    <React.Fragment key={index}>
                      ...
                      >
                        <span className="text-sm">{m.editor_suggestion_with()}</span>
                        <span className="text-sm">{text || m.editor_suggestion_line_breaks()}</span>
                      </div>
                    </React.Fragment>
                  )
                )}
                  ...
                  <React.Fragment key={index}>
                    <div key={index} className="flex items-start gap-2">
                      <span className="text-muted-foreground text-sm">
                        {index === 0 ? m.editor_suggestion_replace() : m.editor_suggestion_delete()}
                      </span>
                      <span className="text-sm">{text || m.editor_suggestion_line_breaks()}</span>
                    </div>
                  </React.Fragment>
