...
export function useLocalizedEmojiI18n(i18n: UseEmojiPickerType['i18n']) {
  return React.useMemo(
    () => ({
      ...i18n,
      categories: {
        ...i18n.categories,
        activity: m.editor_emoji_category_activity(),
        flags: m.editor_emoji_category_flags(),
        foods: m.editor_emoji_category_foods(),
        frequent: m.editor_emoji_category_frequent(),
        nature: m.editor_emoji_category_nature(),
        objects: m.editor_emoji_category_objects(),
        people: m.editor_emoji_category_people(),
        places: m.editor_emoji_category_places(),
        symbols: m.editor_emoji_category_symbols(),
      },
      clear: m.editor_colors_clear(),
      pick: m.editor_emoji_pick(),
      search: m.editor_emoji_search(),
      searchNoResultsSubtitle: m.editor_emoji_no_results_subtitle(),
      searchNoResultsTitle: m.editor_no_results(),
      searchResult: m.editor_emoji_search_result(),
    }),
    [i18n]
  );
}
  ...
  return (
    <EmojiPopover
      control={
        <ToolbarButton
          pressed={isOpen}
          tooltip={m.editor_toolbar_emoji()}
          isDropdown
          {...props}
        >
          <SmileIcon />
...
/**
 * Standalone emoji picker trigger for page chrome without a radix Toolbar
 * (e.g. a Notion-style title bar). Same picker, plain Button trigger.
 */
...
export function EmojiPicker({
  ...
  onMouseOver,
  onSelectEmoji,
}: Omit<UseEmojiPickerType, 'emoji' | 'focusedCategory' | 'icons' | 'settings'> & {
  emoji?: Emoji | undefined;
  focusedCategory?: EmojiCategoryList | undefined;
  icons?: EmojiIconList<React.ReactElement>;
  settings?: UseEmojiPickerType['settings'] | undefined;
}) {
  return (
    <div
      className={cn(
...
function EmojiPreview({ emoji }: { emoji?: Emoji | undefined }) {
  return (
...
function EmojiPickerPreview({
  ...
  isSearching = false,
  ...props
}: Pick<UseEmojiPickerType, 'hasFound' | 'i18n' | 'isSearching'> & {
  emoji?: Emoji | undefined;
}) {
...
function EmojiPickerNavigation({
  ...
  focusedCategory?: EmojiCategoryList | undefined;
} & Pick<UseEmojiPickerType, 'emojiLibrary' | 'i18n' | 'icons'>) {
  return (
