...
export function createModulesRoutes() {
  ...
  app.patch("/api/projects/:projectId/modules/:moduleId", async (context) => {
    const service = new ModulesService(context.var.db);
    return context.json(await service.updateModule(context.var.auth, context.req.param("moduleId"), await context.req.json()));
  });

  app.delete("/api/projects/:projectId/modules/:moduleId", async (context) => {
    const service = new ModulesService(context.var.db);
    return context.json(await service.softDeleteModule(context.var.auth, context.req.param("moduleId")));
  });

  app.post("/api/projects/:projectId/modules/:moduleId/fields", async (context) => {
    const service = new ModulesService(context.var.db);
    return context.json(await service.upsertField(context.var.auth, context.req.param("moduleId"), await context.req.json()), 201);
  });

  app.get("/api/projects/:projectId/modules/:moduleId/records", async (context) => {
    const service = new ModulesService(context.var.db);
    return context.json({ items: await service.listRecords(context.var.auth, context.req.param("moduleId")) });
  });
